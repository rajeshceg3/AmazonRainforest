    ...
    material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        vec3 worldPos = vec3(0.0);
          ...
          worldPos = (modelMatrix * instanceMatrix * vec4(position, 1.0)).xyz;
        #else
        ...
        float barkNoise = fbm(vec2(uv.x * uScale * 5.0, uv.y * uScale));
        ...
        transformed += objectNormal * barkNoise * 0.05;
        `
      )
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        // Bark Texture
        // High freq FBM noise
        float n = fbm(vUv * vec2(20.0, 50.0)); // Stretched Y for vertical bark

        // Moss patches (low freq)
        float moss = fbm(vUv * 5.0 + n * 0.1);
        float mossMask = smoothstep(0.3, 0.7, moss);

        vec3 barkColor = diffuseColor.rgb;
        barkColor *= (0.7 + 0.6 * n); // Variation

        // Lichen spots (high freq)
        float lichen = snoise(vUv * 50.0);
        float lichenMask = smoothstep(0.6, 0.7, lichen);
        vec3 lichenColor = vec3(0.8, 0.9, 0.8);

        vec3 mossColor = vec3(0.2, 0.4, 0.1); // Green

        // Mix
        diffuseColor.rgb = mix(barkColor, lichenColor, lichenMask * 0.3);
        diffuseColor.rgb = mix(diffuseColor.rgb, mossColor, mossMask * 0.6); // Not fully covering
        `
      )
    }
